...
public class OrigemDocumentoController implements com.ine.sge.interfaces.resource.IOrigemDocumento {
	...
	public ResponseEntity<?> show(@Valid @PathVariable Long id){
		verify(id);
		return new ResponseEntity<> (origemdocumentoRepository.findById(id), HttpStatus.OK);
	}
	...
	public ResponseEntity<Page<OrigemDocumento>> showall(Pageable pageable) {
		return new ResponseEntity<>(origemdocumentoRepository.findAll(pageable), HttpStatus.OK);
	}
	...
	public ResponseEntity<Void> update(@Valid @PathVariable Long id, @Valid @RequestBody OrigemDocumento toUpdate){
		...
		Optional<OrigemDocumento> updatedOptionalClass = origemdocumentoRepository.findById(id);
		if (updatedOptionalClass.isPresent()){
			...
			afterIsPresent.setOrigem(toUpdate.getOrigem());
			afterIsPresent.setOrigem_dsg(toUpdate.getOrigem_dsg());
	...
	public ResponseEntity<Void> delete(@Valid @PathVariable Long id, @Valid @RequestBody String lastModifiedBy){
		//		verify(id);
		//		origemdocumentoRepository.deleteById(id);

		Optional<OrigemDocumento> softDelete = origemdocumentoRepository.findById(id);
	...
	private void verify(Long id) throws ResourceNotFoundException {
		origemdocumentoRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Document source with id " + id + " not found"));
	}
	// endregion
