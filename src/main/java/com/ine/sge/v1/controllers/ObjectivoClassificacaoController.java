...
public class ObjectivoClassificacaoController implements com.ine.sge.interfaces.resource.IObjectivoClassificacao {
	...
	public ResponseEntity<?> show(@Valid @PathVariable Long id){
		verify(id);
		return new ResponseEntity<> (objectivoclassificacaoRepository.findById(id), HttpStatus.OK);
	}
	...
	public ResponseEntity<Page<ObjectivoClassificacao>> showall(Pageable pageable) {
		return new ResponseEntity<>(objectivoclassificacaoRepository.findAll(pageable), HttpStatus.OK);
	}
	...
	public ResponseEntity<Void> update(@Valid @PathVariable Long id, @Valid @RequestBody ObjectivoClassificacao toUpdate){
		...
		Optional<ObjectivoClassificacao> updatedOptionalClass = objectivoclassificacaoRepository.findById(id);
	...
	public ResponseEntity<Void> delete(@Valid @PathVariable Long id, @Valid @RequestBody String lastModifiedBy){
		//		verify(id);
		//		objectivoclassificacaoRepository.deleteById(id);

		Optional<ObjectivoClassificacao> softDelete = objectivoclassificacaoRepository.findById(id);
	...
	private void verify(Long id) throws ResourceNotFoundException {
		objectivoclassificacaoRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Goal classification with id " + id + " not found"));
	}
	// endregion
