...
public class TituloAcademicoController implements com.ine.sge.interfaces.resource.ITituloAcademico {
	...
	public ResponseEntity<?> show(@Valid @PathVariable Long id){
		verify(id);
		return new ResponseEntity<> (tituloacademicoRepository.findById(id), HttpStatus.OK);
	}
	...
	public ResponseEntity<Page<TituloAcademico>> showall(Pageable pageable) {
		return new ResponseEntity<>(tituloacademicoRepository.findAll(pageable), HttpStatus.OK);
	}
	...
	public ResponseEntity<Void> update(@Valid @PathVariable Long id, @Valid @RequestBody TituloAcademico toUpdate){
		...
		Optional<TituloAcademico> updatedOptionalClass = tituloacademicoRepository.findById(id);
	...
	public ResponseEntity<Void> delete(@Valid @PathVariable Long id, @Valid @RequestBody String lastModifiedBy){
		//		verify(id);
		//		tituloacademicoRepository.deleteById(id);

		Optional<TituloAcademico> softDelete = tituloacademicoRepository.findById(id);
	...
	private void verify(Long id) throws ResourceNotFoundException {
		tituloacademicoRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Degree with id " + id + " not found"));
	}
	// endregion
