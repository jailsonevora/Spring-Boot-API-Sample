...
public class FormaJuridicaController implements com.ine.sge.interfaces.resource.IFormaJuridica {
	...
	public ResponseEntity<?> show(@Valid @PathVariable Long id){
		verify(id);
		return new ResponseEntity<> (formaJuridicaRepository.findById(id), HttpStatus.OK);
	}
	...
	public ResponseEntity<Page<FormaJuridica>> showall(Pageable pageable) {
		return new ResponseEntity<>(formaJuridicaRepository.findAll(pageable), HttpStatus.OK);
	}
	...
	public ResponseEntity<Void> update(@Valid @PathVariable Long id, @Valid @RequestBody FormaJuridica toUpdate){
		...
		Optional<FormaJuridica> updatedOptionalClass = formaJuridicaRepository.findById(id);
...
//		verify(id);
// 		formaJuridicaRepository.deleteById(id);

		Optional<FormaJuridica> softDelete = formaJuridicaRepository.findById(id);
	...
	private void verify(Long id) throws ResourceNotFoundException {
		formaJuridicaRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Legal form with id " + id + " not found"));
	}
	// endregion
