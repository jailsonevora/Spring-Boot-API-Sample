...
import com.ine.sge.dao.ISocioRepository;
...
@Api(value = "socios", description = "Socio API")
public class SocioController implements com.ine.sge.interfaces.resource.ISocio {
	...
	public ResponseEntity<?> show(@Valid @PathVariable Long id){
		verify(id);
		return new ResponseEntity<> (socioRepository.findById(id), HttpStatus.OK);
	}
	...
	public ResponseEntity<Page<Socio>> showall(Pageable pageable) {
		return new ResponseEntity<>(socioRepository.findAll(pageable), HttpStatus.OK);
	}
	...
	public ResponseEntity<Void> update(@Valid @PathVariable Long id, @Valid @RequestBody Socio toUpdate){
		...
		Optional<Socio> updatedOptionalClass = socioRepository.findById(id);
	...
	public ResponseEntity<Void> delete(@Valid @PathVariable Long id, @Valid @RequestBody String lastModifiedBy){
		...
		Optional<Socio> softDelete = socioRepository.findById(id);
	...
	private void verify(Long id) throws ResourceNotFoundException {
		socioRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Partner with id " + id + " not found"));
	}
	// endregion
