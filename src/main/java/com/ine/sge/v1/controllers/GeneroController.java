...
public class GeneroController implements com.ine.sge.interfaces.resource.IGenero {
	...
	public ResponseEntity<?> show(@Valid @PathVariable Long id){
		verify(id);
		return new ResponseEntity<> (generoRepository.findById(id), HttpStatus.OK);
	}
	...
	public ResponseEntity<Page<Genero>> showall(Pageable pageable) {
		return new ResponseEntity<>(generoRepository.findAll(pageable), HttpStatus.OK);
	}
	...
	public ResponseEntity<Void> update(@Valid @PathVariable Long id, @Valid @RequestBody Genero toUpdate){
		...
		Optional<Genero> updatedOptionalClass = generoRepository.findById(id);
...
// 		generoRepository.deleteById(id);
		...
		Optional<Genero> softDelete = generoRepository.findById(id);
	...
	private void verify(Long id) throws ResourceNotFoundException {
		generoRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Gender with id " + id + " not found"));
	}
	// endregion
}
