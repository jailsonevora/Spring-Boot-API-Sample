...
public class ContabilidadeController implements com.ine.sge.interfaces.resource.IContabilidade {
	...
	public ResponseEntity<?> show(@Valid @PathVariable Long id){
		verify(id);
		return new ResponseEntity<> (contabilidadeRepository.findById(id), HttpStatus.OK);
	}
	...
	public ResponseEntity<Page<Contabilidade>> showall(Pageable pageable) {
		return new ResponseEntity<>(contabilidadeRepository.findAll(pageable), HttpStatus.OK);
	}
	...
	public ResponseEntity<Void> update(@Valid @PathVariable Long id, @Valid @RequestBody Contabilidade toUpdate){
		...
		Optional<Contabilidade> updatedOptionalClass = contabilidadeRepository.findById(id);
		if (updatedOptionalClass.isPresent()){
			...
			afterIsPresent.setCnta_dsg(toUpdate.getCnta_dsg());
...
// 		contabilidadeRepository.deleteById(id);
		...
		Optional<Contabilidade> softDelete = contabilidadeRepository.findById(id);
	...
	private void verify(Long id) throws ResourceNotFoundException {
		contabilidadeRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Accounting with id " + id + " not found"));
	}
	// endregion
