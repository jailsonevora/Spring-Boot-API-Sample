...
public class CanalRecepcaoController implements com.ine.sge.interfaces.resource.ICanalRecepcao{
	...
	public ResponseEntity<?> show(@Valid @PathVariable Long id){
		verify(id);
		return new ResponseEntity<> (canalRecepcaoRepository.findById(id), HttpStatus.OK);
	}
	...
	public ResponseEntity<Page<Canal>> showall(Pageable pageable) {
		return new ResponseEntity<>(canalRecepcaoRepository.findAll(pageable), HttpStatus.OK);
	}
	...
	public ResponseEntity<Void> update(@Valid @PathVariable Long id, @Valid @RequestBody Canal toUpdate){
		...
		Optional<Canal> updatedOptionalClass = canalRecepcaoRepository.findById(id);
...
//		canalRecepcaoRepository.deleteById(id);
		...
		Optional<Canal> softDelete = canalRecepcaoRepository.findById(id);
	...
	private void verify(Long id) throws ResourceNotFoundException {
		canalRecepcaoRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Reception channel with id " + id + "not found"));
	}
	// endregion
